...
async function save() {
  ...
  try {
    ...
    if (form.value.secret_key.trim()) patch.secret_key = form.value.secret_key.trim()
    if (form.value.passphrase.trim()) patch.passphrase = form.value.passphrase.trim()
  ...
  >
    <div class="flex flex-col gap-4 py-2">
      <div class="flex items-center gap-2">
        <Checkbox v-model="form.enabled" inputId="edit-enabled" :binary="true" />
        <label for="edit-enabled" class="text-sm cursor-pointer text-surface-700 dark:text-surface-300">Enabled</label>
      </div>
      ...
      <div v-if="!isWalletBased" class="flex flex-col gap-2">
        ...
        <InputText v-model="form.api_key" placeholder="Leave empty to keep current" />
      </div>
      ...
      <div class="flex items-center gap-2 mt-2">
        <Checkbox v-model="form.testnet" inputId="edit-testnet" :binary="true" />
        <label for="edit-testnet" class="text-sm cursor-pointer text-surface-700 dark:text-surface-300">Use testnet</label>
      </div>
    </div>
    ...
    <template #footer>
      <div class="flex justify-end gap-2 mt-4">
        ...
        <Button :label="saving ? 'Saving…' : 'Save'" icon="pi pi-check" :loading="saving" :disabled="!canSubmit" @click="save" />
      </div>
    </template>
  </Dialog>
</template>
