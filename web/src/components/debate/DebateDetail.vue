...
<template>
  <div
    class="flex flex-col gap-6 bg-surface-50 dark:bg-surface-900 border border-surface-200 dark:border-surface-800 rounded-2xl p-6 transition-colors duration-300"
  >
    <!-- Header -->
    <div
      class="flex items-center justify-between border-b border-surface-200 dark:border-surface-800 pb-4 flex-wrap gap-4"
    >
      <div>
        <h2 class="font-bold text-xl text-surface-900 dark:text-white">
          {{ debate.name || debate.symbol }}
        </h2>
        <div
          class="flex items-center gap-2 mt-1 text-xs text-surface-500 dark:text-surface-400"
        >
          <span>Round {{ debate.current_round }}/{{ debate.max_rounds }}</span>
          <span>·</span>
          <span
            class="capitalize px-2 py-0.5 rounded-md font-semibold text-xs tracking-wider"
            :class="{
    ...
    >
      <h3
        class="font-bold text-sm text-primary-600 dark:text-primary-400 mb-2 flex items-center gap-1.5"
      >
      ...
      <div class="flex items-start gap-4">
        <span
          class="text-3xl font-black text-primary-600 dark:text-primary-400 select-none"
          >{{ debate.final_decision }}</span
        >
        <span
          class="text-xs text-surface-600 dark:text-surface-400 leading-relaxed pt-1.5"
        >
    ...
    <div>
      <h3
        class="font-bold text-sm text-surface-700 dark:text-surface-300 mb-3 flex items-center gap-2"
      >
      ...
      <div class="flex flex-col gap-3 max-h-125 overflow-auto pr-1">
        ...
        >
          <div class="flex items-center gap-2 mb-1.5">
            ...
            <span
              class="font-bold capitalize text-surface-800 dark:text-surface-200"
              >{{ message.personality }}</span
            >
            <span
              class="text-xs ml-auto text-surface-500 dark:text-surface-500 font-medium"
            >
              Round {{ message.round }}
            </span>
            <span
          ...
          <p
            class="text-xs leading-relaxed text-surface-600 dark:text-surface-300"
          >
            {{ message.content }}
          </p>
        </div>
