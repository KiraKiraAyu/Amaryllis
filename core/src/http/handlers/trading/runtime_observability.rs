...
pub async fn runtime_events(
    ...
    let payload = trading_service(&app).runtime_events(
        q.trader_id, q.window_hours, q.limit, q.offset, q.event_type, q.risk_level, q.correlation_id,
    ).await?;
...
pub async fn runtime_metrics(
    ...
    let payload = trading_service(&app).runtime_metrics(q.trader_id, q.window_hours).await?;
    Ok(Json(ApiResponse::success(Some(payload), None)))
}
